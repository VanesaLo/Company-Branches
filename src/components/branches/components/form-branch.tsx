...
import { createBranch, updateBranch } from "app/app/actions/branches.action";
...
const FormMapDraggable = dynamic(() => import("./form-map-draggable"), {
  ssr: false,
...
export default function FormBranch({ setOpen, branch }: BranchFormProps) {
  // -- Hooks
  ...
  // -- States Save
  const [formState, formBranchAction, isPending] = useActionState<
    ActionState,
    BranchSchema
  >(createBranch, { error: "", success: "" });

  const [formStateUpdate, formBranchUpdateAction, isPendingUpdate] =
    useActionState<ActionState, BranchUpdateSchema>(updateBranch, {
      error: "",
      success: "",
    });
  ...
  const errorSaving = useMemo(() => {
    return formState.error || formStateUpdate.error;
  }, [formState.error, formStateUpdate.error]);

  // -- Form
  const form = useForm<BranchSchema | BranchUpdateSchema>({
    resolver: zodResolver(!branch ? branchSchema : branchUpdateSchema),
    defaultValues: branch
      ? {
          ...branch,
          pais: branch.pais_id.toString(),
          departamento: branch.estado_id.toString(),
          city_id: branch.ciudad_id.toString(),
          imagen: "",
        }
      : undefined,
  });
  ...
  async function onErrorForm(errors: any) {
    toast.error("Please fill all the required fields");
    console.log({ errors });
  }

  async function onSubmit(values: BranchSchema | BranchUpdateSchema) {
    try {
      startTransition(() => {
        if (branch) formBranchUpdateAction(values as BranchUpdateSchema);
        else formBranchAction(values as BranchSchema);
      });
    } catch (error) {
  ...
  useEffect(() => {
    ...
    if (branch) {
      getDepartments(branch.pais_id.toString());
      getCities(branch.estado_id.toString());
    }
  }, [branch]);
  ...
  useEffect(() => {
    if (isSubmitSuccessful)
      if (formState.success) successSaved("Branch created successfully");
      else if (formStateUpdate.success)
        successSaved("Branch updated successfully");
  }, [
    successSaved,
    isSubmitSuccessful,
    formState.success,
  ...
  return (
    <>
      <Form {...form}>
        <form
          onSubmit={handleSubmit(onSubmit, onErrorForm)}
          className="space-y-4"
        >
          ...
          {errorSaving ? (
            <div className="text-red-500 text-sm">{errorSaving}</div>
          ) : null}
